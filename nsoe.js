  ...
  const observer = new MutationObserver((mutations) => {
    {
      ...
      document.querySelectorAll(".sf_DialogTitle").forEach((element) => {
        if (element.innerText.match(/application/i)) {
          element.innerText = element.innerText.replace("application", "request").replace("Application", "Request").replace("an Application", "a Request");
        }
      });
      ...
      document.querySelectorAll(".sf_DialogHtml").forEach((element) => {
        if (element.innerHTML.match(/application/i)) {
          element.innerHTML = element.innerHTML.replace("an Application", "a Request").replace("Application", "Request").replace("application", "request");
...

{
  ...
  function completeStep2Custom(pStep, pMoveOn, pMoveToOriginalStep, event) {
    ...
    if (finalValidate("step", vVal)) {
      document.getElementById("Complete" + pStep).innerHTML =
        "<img src='/webspeed/green-checkmark.png' alt='Completed' style='width:14px;vertical-align:-3px'/>Date Completed: 11/14/2023";
...
const show = {
  ...
  schoolYearApplyingFor: (visible) => {
    if (visible) {
      ...
      document.getElementById("StuInfoExpectedDateofEnroll").value = new Date().toLocaleDateString("en-US", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
      });
...
const requestFor = {
  ...
  schoolVisit: (selected) => {
    if (selected) {
      ...
      document.querySelector("#Step6 .instructarea").innerHTML = document
        .querySelector("#Step6 .instructarea")
        .innerHTML.replace("Complete Step 6", "Complete Step 3");
      // Enable Calendly
      document.querySelector(
        "#afterSubmitMessage"
      ).innerHTML = `<div class="calendly-inline-widget" data-url="https://calendly.com/d/3h3-4z3-r47/meeting-visit-request" style="min-width:320px;height:961px;"></div><script type="text/javascript" src="https://assets.calendly.com/assets/external/widget.js" async><\/script>`;
      ...
      document.querySelector("#Step6 .instructarea").innerHTML = document
        .querySelector("#Step6 .instructarea")
        .innerHTML.replace("Complete Step 3", "Complete Step 6");
      // Disable Calendly
  ...
  summerCamp: (selected) => {
    if (selected) {
      ...
      defaultGrade = () => {
        ...
        if (vToday.getMonth() < vBirthdate.getMonth() || (vToday.getMonth() == vBirthdate.getMonth() && vToday.getDate() < vBirthdate.getDate())) {
          vAge -= 1;
        }
        document.getElementById("tAge").value = vAge;
        if (document.getElementById("tAge").value == "NaN") document.getElementById("tAge").value = "0";
      ...
      document.querySelector("#Step6 .instructarea").innerHTML = document
        .querySelector("#Step6 .instructarea")
        .innerHTML.replace("Complete Step 6", "Complete Step 5");
      ...
      document.querySelector("#afterSubmitMessage").innerHTML = `<div style="text-align:center;">You will receive an answer within one business day.
            <br><br>
      ...
      document.querySelector("#Step6 .instructarea").innerHTML = document
        .querySelector("#Step6 .instructarea")
        .innerHTML.replace("Complete Step 5", "Complete Step 6");
